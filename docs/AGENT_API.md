...
| `/api/agent/v1/director/sotuv?from=&to=` | Sotuv bazasi (КП/договорные цены): маржа против официального тарифа. KPI (jami KP, hajm, kelishuv summasi, marja, zarar KP), sellers (маржа по продавцам), deals (худшая маржа первой; в компакте топ-40, `deals_total` = всего): kelishuv vs base («Uslugasiz» минимум) / list («Price narx»), margin_m3, margin_total, status zarar/past/ok. |
