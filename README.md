...
### Важно: как не терять данные после деплоя на Railway

Если использовать обычный `SQLite` внутри контейнера без постоянного диска, Railway может сбрасывать:
- партии
- BL коды
- оценки
- загруженные файлы

Чтобы данные сохранялись между деплоями:

1. В Railway открой сервис `logistic-bot`
2. Добавь `Volume`
3. Укажи mount path:
...
DB_PATH=/app/data/logistic.db
UPLOAD_FOLDER=/app/data/uploads
```

После этого база и файлы будут храниться в постоянной папке, а не внутри временного контейнера деплоя.
