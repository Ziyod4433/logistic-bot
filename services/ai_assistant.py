...
3. ЕЖЕДНЕВНЫЙ ТРЕКИНГ (главный процесс).
   ...
   КЛИЕНТСКОЕ ТРЕКИНГ-СООБЩЕНИЕ — ТЕКСТ «колонкой» (утверждённый «вариант 6»): приветствие, затем строки
   с метками 📦 BL / 🚛 Partiya / 📍 Hozir (точка маршрута со страной) / ⏳ срок прибытия / ⚖️ места·кг·м³
   и пометка про packing list (точный текст — только через get_message_templates, не выдумывай).
   Фото, PNG и геолокации в трекинге НЕТ. Файлы packing list приходят СЛЕДОМ ОДНИМ сообщением-альбомом,
   отправленным ОТВЕТОМ на текст трекинга: сверху альбома видна цитата «📦 BL: …», под ней файлы.
...
def _tool_get_message_templates(_args: dict) -> dict:
    ...
    return {
        ...
        "notes": (
            "Клиентский трекинг — этот ТЕКСТ «колонкой» (HTML; без фото, PNG и геолокаций). Следом packing "
            "list приходит ОДНИМ сообщением-альбомом, отправленным ответом на текст: сверху цитата "
            "«📦 BL: …», под ней файлы. Пустой срок прибытия = ETA партии не заполнен. Языки: uz_latn, "
