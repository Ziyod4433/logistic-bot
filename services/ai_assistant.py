...
3. ЕЖЕДНЕВНЫЙ ТРЕКИНГ (главный процесс).
   ...
   КЛИЕНТСКОЕ ТРЕКИНГ-СООБЩЕНИЕ = ЖИВАЯ КАРТА + ТЕКСТ. Сверху — нативная live-геолокация Telegram:
   маркер фуры на настоящей карте по городу текущего статуса (Yiwu/Zhongshan → Horgos → Almata → … → Toshkent;
   позиция по городам, НЕ GPS с машины). На (чат, партию) — ОДНО такое сообщение: при новом статусе бот
   ПЕРЕДВИГАЕТ маркер (editMessageLiveLocation), по прибытии в Toshkent live останавливается.
   Под картой — текстовый шаблон «вариант 7»: срок прибытия, линия маршрута, текущая точка со страной,
   реквизиты одной строкой (точный текст — только через get_message_templates, не выдумывай). Фото/PNG в
   трекинге НЕТ. Файлы packing list — внутри текстового сообщения (caption) или сразу после него.
...
def _tool_get_message_templates(_args: dict) -> dict:
    ...
    return {
        ...
        "notes": (
            "Клиентский трекинг = ЖИВАЯ КАРТА (нативная live-геолокация Telegram, маркер фуры по городу "
            "статуса; одно карт-сообщение на чат+партию, при новом статусе маркер передвигается) + этот "
            "ТЕКСТ под ней (HTML, без фото/PNG). Packing list — внутри текстового сообщения (caption) или "
            "сразу после. Пустой срок прибытия = ETA партии не заполнен. Языки: uz_latn, uz_cyrl, ru, en — "
            "по языку группы."
        ),
