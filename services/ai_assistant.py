...
3. ЕЖЕДНЕВНЫЙ ТРЕКИНГ (главный процесс).
   ...
   КЛИЕНТСКОЕ ТРЕКИНГ-СООБЩЕНИЕ — ОДНО сообщение: альбом файлов packing list, под ним подпись —
   текст шаблона сайта (раздел «Шаблон сообщения»; порядок: Joriy holati → срок прибытия → Partiya →
   BL-kod → данные груза; строки «Bugungi sana» больше НЕТ; точный текст — только через
   get_message_templates, не выдумывай). В подписи приветствие и строка «🖇 packing list» — обычным
   текстом, всё между ними — нативной цитатой-«окном» (blockquote). Фото, PNG и геолокаций НЕТ.
   Без файлов (или подпись >1024 символов) текст уходит отдельным сообщением, файлы следом без цитат.
...
def _tool_get_message_templates(_args: dict) -> dict:
    ...
    return {
        ...
        "notes": (
            "Клиентский трекинг — ОДНО сообщение: альбом packing list, под файлами этот текст подписью; "
            "приветствие и строка «🖇 packing list» обычные, середина — цитатой-«окном» (blockquote). "
            "Без фото, PNG и геолокаций. Пустой срок прибытия = ETA партии не заполнен. Языки: uz_latn, "
            "uz_cyrl, ru, en — по языку группы."
        ),
