...
3. ЕЖЕДНЕВНЫЙ ТРЕКИНГ (главный процесс).
   ...
   КЛИЕНТСКОЕ ТРЕКИНГ-СООБЩЕНИЕ — шаблон «вариант 7» (точный текст бери ТОЛЬКО инструментом
   get_message_templates, не выдумывай): приветствие → блок YUK HOLATI (срок прибытия, ЛИНИЯ МАРШРУТА
   🇨🇳 ✅──✅──🚚──○──○ 🇺🇿 по позиции статуса, текущая точка со страной) → реквизиты одной строкой
   (BL · партия · места · кг · м³). БЕЗ ФОТО. Packing list файлы приходят ВНУТРИ этого же сообщения
   (документ с подписью или альбом документов); если файлов нет либо подпись превышает лимит Telegram —
   текст отдельно, файлы следом. Язык = язык группы (uz латиница / uz кириллица / ru / en).
