...
3. ЕЖЕДНЕВНЫЙ ТРЕКИНГ (главный процесс).
   ...
   КЛИЕНТСКОЕ ТРЕКИНГ-СООБЩЕНИЕ — первоначальный текстовый шаблон сайта (редактируется в разделе
   «Шаблон сообщения»; точный текст — только через get_message_templates, не выдумывай).
   Фото, PNG и геолокации в трекинге НЕТ. Файлы packing list приходят СЛЕДОМ ОДНИМ сообщением-альбомом,
   отправленным ОТВЕТОМ на текст трекинга с цитатой ВСЕГО текста: сверху альбома — весь трекинг
   в виде цитаты-«окна», под ним файлы. Язык = язык группы (uz латиница / uz кириллица / ru / en).
...
def _tool_get_message_templates(_args: dict) -> dict:
    ...
    return {
        ...
        "notes": (
            "Клиентский трекинг — этот ТЕКСТ (первоначальный шаблон сайта, HTML; без фото, PNG и "
            "геолокаций). Следом packing list приходит ОДНИМ сообщением-альбомом, отправленным ответом "
            "на текст с цитатой ВСЕГО трекинга: сверху альбома весь текст в виде цитаты-«окна», под ним "
            "файлы. Пустой срок прибытия = ETA партии не заполнен. Языки: uz_latn, uz_cyrl, ru, en — "
            "по языку группы."
        ),
