...
3. ЕЖЕДНЕВНЫЙ ТРЕКИНГ (главный процесс).
   ...
   КЛИЕНТСКОЕ ТРЕКИНГ-СООБЩЕНИЕ — ОДНО сообщение: текст шаблона сайта (раздел «Шаблон сообщения»;
   порядок: Joriy holati → срок прибытия → Partiya → BL-kod → данные груза; строки «Bugungi sana»
   больше НЕТ; точный текст — только через get_message_templates, не выдумывай), где приветствие и
   строка «🖇 packing list» обычные, а всё между ними — нативной цитатой-«окном» (blockquote).
   ПОД текстом — файлы packing list КНОПКАМИ (одна кнопка = один файл с его именем; нажатие скачивает
   файл с сайта по публичной ссылке /public/file/<token>). Фото, PNG и геолокаций НЕТ.
   Запасные пути: нет публичных ссылок → альбом документов с текстом в подписи (файлы над текстом);
   иначе текст отдельно + файлы следом. Язык = язык группы (uz латиница / uz кириллица / ru / en).
...
def _tool_get_message_templates(_args: dict) -> dict:
    ...
    return {
        ...
        "notes": (
            "Клиентский трекинг — ОДНО сообщение: этот текст (приветствие и строка «🖇 packing list» "
            "обычные, середина — цитатой-«окном» blockquote), а ПОД текстом файлы packing list "
            "КНОПКАМИ — одна кнопка = один файл, нажатие скачивает его с сайта. Без фото, PNG и "
