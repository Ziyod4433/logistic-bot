...
3. ЕЖЕДНЕВНЫЙ ТРЕКИНГ (главный процесс).
   ...
   КЛИЕНТСКОЕ ТРЕКИНГ-СООБЩЕНИЕ — PNG-КАРТОЧКА («окно» в фирменном тёмном стиле): заголовок YUK HOLATI,
   крупный срок прибытия, нарисованная линия маршрута с галочками и позицией фуры, текущая точка со страной,
   внизу панель реквизитов (BL · партия · места · кг · м³). Подпись под фото — реквизиты + пометка про
   packing list. Файлы packing list приходят ОТДЕЛЬНЫМИ сообщениями ВНИЗУ, после карточки. Запасной путь
   при сбое рендера: текстовый шаблон «вариант 7» (точный текст — только через get_message_templates,
   не выдумывай). Язык = язык группы (uz латиница / uz кириллица / ru / en).
